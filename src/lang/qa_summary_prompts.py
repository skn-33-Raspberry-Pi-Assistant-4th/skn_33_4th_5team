...
3. 답변의 핵심을 한국어 1~3문장, 한 줄, 500자 이내로 요약하세요.
4. 질문의 모든 요구를 다시 답하려 하지 말고, 원답변이 실제로 설명한 핵심 내용만 압축하세요. 원답변에 빠진 항목은 추측하여 채우지 마세요.
...
TITLE_REVIEW_SYSTEM_PROMPT = """질문과 제목을 검수합니다. 질문에 없는 장치·부품·기능 명칭이나 의미가 다른 기술 용어가 제목에 있으면 valid=false입니다. 질문의 핵심 주제와 요청 의도가 보존되지 않아도 valid=false입니다. 질문과 직접 모순되거나 질문에 없는 내용이 추가된 경우에만 false로 판단하세요. JSON 객체 {"valid":true} 또는 {"valid":false} 하나만 출력하세요."""
...
관련 없는 부가 정보를 핵심 답처럼 요약해도 false입니다. 원답변의 오류를 요약이 그대로 옮기면 false입니다. 원답변과 직접 모순되거나 원답변·인용 근거에 없는 내용이 추가된 경우에만 false로 판단하세요.
