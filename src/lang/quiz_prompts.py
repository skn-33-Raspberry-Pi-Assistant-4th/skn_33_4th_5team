...
4. 최대 요청된 문항 수만 생성하세요. 1~최대 문항 수만큼 만들 수 있으며, 한 문항도 충분히 근거 있게 만들 수 없으면 status를 "insufficient_content"으로 하고 questions는 빈 배열로 반환하세요.
...
6. 각 문항은 evidence_ids에 허용된 evidence ID 하나만 넣고, supporting_quotes에는 그 evidence 원문에서 연속된 문자열을 그대로 복사한 문장 하나만 넣으세요. supporting_quote는 정답과 explanation을 직접 뒷받침해야 하며, explanation은 그 범위를 넘어서면 안 됩니다.
...
8. 같은 핵심 사실을 표현만 바꿔 여러 문항으로 반복하지 마세요. 오답은 사전 지식으로 새 사실을 만들지 말고, 가능하면 제공된 자료의 개념·대상·속성을 잘못 연결해 구성하세요. 제공된 자료만으로 오답임을 판단할 수 있어야 하며, 선택지는 서로 중복되거나 사실상 같은 의미가 아니어야 합니다. 정답만 유난히 길거나 구체적이지 않게 하세요.
  ...
  "questions": [
    {
      ...
      "explanation": "...",
      "evidence_ids": ["C1"],
      "supporting_quotes": ["evidence 원문에서 그대로 복사한 연속 문자열"]
    }
  ]
}"""
