...
6. 각 문항의 evidence_ids에는 허용된 evidence ID 하나만 넣으세요. supporting_quote_id에는 아래 허용된 quote 후보 중 해당 evidence ID에 속한 ID 하나만 넣으세요. quote 원문은 서버가 ID로 처리하므로 supporting_quotes 필드를 출력하지 마세요. explanation은 선택한 quote 후보와 answer의 범위를 넘어서면 안 됩니다.
...
def _render_quote_candidates(request: QuizGenerationRequest) -> str:
    return "\n".join(
        f'<quote_candidate id="{escape(candidate.quote_id, quote=True)}" '
        f'evidence_id="{escape(candidate.evidence_id, quote=True)}">'
        f"{escape(candidate.content)}</quote_candidate>"
        for candidate in request.quote_candidates
    )
...
<allowed_quote_candidates>
{_render_quote_candidates(request)}
</allowed_quote_candidates>
