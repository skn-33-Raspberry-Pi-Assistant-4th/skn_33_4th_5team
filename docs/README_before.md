...
`.env`에 MySQL 비밀번호와 Django Secret을 설정한 뒤 실행합니다. 자세한 환경 변수와 초기화 방법은 [Django 실행 가이드](README_django_실행법.md)를 따릅니다. 처음 clone한 팀원은 `python scripts/init.py`로 같은 초기 설정을 자동화할 수 있습니다.
...
Qwen3-4B 4-bit 환경에서 고정 fixture 25건을 평가했습니다. `available` 20건과 근거 부족 5건을 포함하며, 상세 조건과 실패 사례는 [Mini Challenge 최종 평가](validation/mini-challenge-final-evaluation.md)에 기록했습니다.
...
Command Lab은 확장된 23개 문서·381개 청크 기준으로 최종 검수된 명령어 100개를 모두 제공합니다. 주제별 탐색·검색, 구성 요소 분석, 안전한 입력값 재조합과 공식 근거 연결을 지원하며 카탈로그 audit과 서비스·UI 회귀 검증을 완료했습니다. 각 수치는 평가셋과 실행 조건에 종속되므로 다른 데이터셋의 결과와 직접 비교할 때는 분모와 평가 버전을 함께 확인해야 합니다.

## 문서

- [Django 모델 계약](data-contracts/portal-models.md)
- [Command Lab 계약·검증](data-contracts/command-lab.md)
- [Mini Challenge 인계 계약](data-contracts/mini-challenge-handoff.md)
