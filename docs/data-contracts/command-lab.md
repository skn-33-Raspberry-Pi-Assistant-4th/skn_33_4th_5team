...
편집 가능한 22개 필드는 SSH 대상, 네트워크 주소·포트, Wi-Fi SSID, 키보드 행·열,
TGA 이미지 경로 유형으로 검증한다. 서버가 최종 판정을 담당하며 Django 화면은
`POST /api/lab/compose`를 사용해 입력 중 같은 판정을 표시한다.

- `valid`: 공식 검수 예제를 변경하지 않은 상태
- `warning`: 구조·형식·범위 검사를 통과한 사용자 변경값. 실제 장치·계정·네트워크 존재 여부와 실행 성공은 미확인
- `error`: 형식 또는 안전 규칙 위반. 재조립과 서랍 저장을 허용하지 않음

IPv4·IPv6와 호스트 이름 형식, Linux 사용자명, `1~65535` 포트 범위, SSID 32바이트,
0 이상의 행·열, `.tga` 확장자와 상위 경로 사용 여부를 확인한다. 셸 연산자와 제어 문자도
계속 차단한다. 오류 응답은 `field`에 `part_id`를 포함하므로 화면이 해당 입력칸에 구체적인
메시지를 표시할 수 있다. 재조립 결과의 변경된 구성 요소는 `changed=true`로 표시한다.

이 검증은 명령 구조와 입력 형식 판정이다. 서버에 접속하거나 명령을 실행하지 않으므로
호스트 도달 가능성, 계정 존재 여부, 장치별 키 행·열 지원 여부와 실제 실행 성공은 판정하지 않는다.
