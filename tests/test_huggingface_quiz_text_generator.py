...
def test_quiz_text_generator_uses_structured_token_default() -> None:
    answer_generator = Mock(spec=HuggingFaceAnswerGenerator)
    result = GenerationResult(
        text='{"status":"insufficient_content","questions":[]}',
        provider="huggingface",
    ...
    answer_generator.generate_structured.return_value = result
    generator = HuggingFaceQuizTextGenerator(answer_generator)
    messages = [{"role": "system", "content": "JSON만 출력하세요."}]

    generator.generate(messages)

    answer_generator.generate_structured.assert_called_once_with(messages, max_new_tokens=1024)
